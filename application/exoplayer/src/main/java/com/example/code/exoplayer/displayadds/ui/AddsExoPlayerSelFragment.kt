...
@AndroidEntryPoint
class AddsExoPlayerContentSelFragment : BottomSheetDialogFragment(), CoroutineScope {

    private val job = Job()
    override val coroutineContext: CoroutineContext
        get() = job + Dispatchers.Main

    private var listener : AddsPlayerCallback? = null

    private val binding by lazy(LazyThreadSafetyMode.NONE) {
        FragmentExoPlayerContentSelectionBinding.inflate(layoutInflater)
    }

    fun setOnClickListener(listener: AddsPlayerCallback) { this.listener = listener }
