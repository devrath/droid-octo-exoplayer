...
@AndroidEntryPoint
class SegmentedExoPlayerSelFragment : BottomSheetDialogFragment(), CoroutineScope {

    private val job = Job()
    override val coroutineContext: CoroutineContext
        get() = job + Dispatchers.Main

    private var listener : SegmentedPlayerCallback? = null

    private val binding by lazy(LazyThreadSafetyMode.NONE) {
        FragmentExoPlayerContentSelectionBinding.inflate(layoutInflater)
    }

    fun setOnClickListener(listener: SegmentedPlayerCallback) { this.listener = listener }
...
interface SegmentedPlayerCallback {
